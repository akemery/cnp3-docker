...
R2, R3 and R4 such as trafic from A->B and B->A goes throught R2 and R3.
"""
        

class StaticECMPRouting(IPTopo):
    ...
    def build(self, *args, **kwargs):
        ...
        super(StaticECMPRouting, self).build(*args, **kwargs)
    ...
    def pre_stop(self, net):
        ...
        nodes = ["R1"]
        outIntf=["R1-eth1", "R1-eth1"]
        dst = ["192.168.4.0/24", "192.168.5.0/24"]
        nexthopIP = ["192.168.3.2", "192.168.3.2"]
        ...
        dst = "192.168.2.0/24"
        nexthopIP = ["192.168.3.2", "192.168.6.2"]
        state, feedback = test.check_ecmp_route(self.family, dst, outIntf, nexthopIP, nodes)
        print(state, file=open("state3.txt", "a"))
        print(feedback, file=open("feedback3.txt", "a"))
        ...
        dst = ["192.168.3.0/24", "192.168.6.0/24"]
        nexthopIP = ["192.168.5.2", "192.168.5.2"]
        state, feedback = test.check_static_route(self.family, dst, outIntf, nexthopIP, nodes)
        print(state, file=open("state3.txt", "a"))
        print(feedback, file=open("feedback3.txt", "a"))
        
        nodes = ["R4"]
        outIntf=["R4-eth1", "R4-eth2"]
        dst = "192.168.1.0/24"
        nexthopIP = ["192.168.4.2", "192.168.5.2"]
        state, feedback = test.check_ecmp_route(self.family, dst, outIntf, nexthopIP, nodes)
        print(state, file=open("state3.txt", "a"))
        print(feedback, file=open("feedback3.txt", "a"))
       
        super(StaticECMPRouting, self).pre_stop(net)
...
net = IPNet(topo=StaticECMPRouting("4", subnet, prefixlen, gw), use_v4=False, use_v6=False, allocate_IPs=False)
